...
<section class="hero">
  <div class="container vcenter">
    <div class="shortener-50">
      <h1 class="heading-style-h1 split-type bugfix-width">
        <?php
        if (is_category()) {
          single_cat_title();
        } elseif (is_tag()) {
          ...
          printf(__('Author: %s', 'enhancethat'), '<span class="vcard">' . get_the_author() . '</span>');
        } elseif (is_day()) {
          printf(__('Day: %s', 'enhancethat'), '<span>' . get_the_date() . '</span>');
        } elseif (is_month()) {
          printf(__('Month: %s', 'enhancethat'), '<span>' . get_the_date('F Y') . '</span>');
        } elseif (is_year()) {
          printf(__('Year: %s', 'enhancethat'), '<span>' . get_the_date('Y') . '</span>');
        } else {
          _e('Archives', 'enhancethat');
        }
        ?>
        <span class="font-family-domaine"></span>
      </h1>
      <?php
      ...
      if ($description) :
      ?>
      <div class="archive-description text-style-regular text-color-white" style="margin-top: 20px; max-width: 600px; margin-left: auto; margin-right: auto;">
...
<section class="section">
  <div class="container">
    ...
    <?php else : ?>
      <p class="text-style-regular">No posts found in this archive.</p>
    <?php endif; ?>
  </div>
</section>
