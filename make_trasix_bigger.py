#!/usr/bin/env python3
"""
Make trasix logo another 20% bigger (total 1.44x original size)
"""
import json
import base64
import shutil
...
def make_trasix_bigger():
    """Make trasix 20% bigger while keeping green color"""

    # Load the original image
    img = Image.open('trasix.png').convert('RGBA')
    print(f"[OK] Loaded trasix.png: {img.size}")

    # Make it 44% bigger total (1.2 * 1.2 = 1.44)
    new_size = int(img.width * 1.44), int(img.height * 1.44)
    img_resized = img.resize(new_size, Image.Resampling.LANCZOS)
    print(f"[OK] Resized to: {img_resized.size} (44% bigger than original)")
    ...
    canvas_size = 280  # Larger canvas for bigger logo
    ...
    # Backup
    backup = f'{animation_file}.backup_bigger_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    ...
    for asset in anim_data['assets']:
        if asset.get('id') == 'image_0':
            ...
            print(f"[OK] Updated image_0 asset with bigger trasix logo")
            break
    ...
    print(f"\n[OK] SUCCESS!")
    print(f"  - Made logo 44% bigger than original (20% more than before)")
    print(f"  - Color: dark green RGB(23, 72, 12)")
    print(f"  - Centered logo in {canvas_size}x{canvas_size} canvas")
    print(f"\nReload the page to see the bigger trasix logo!")
